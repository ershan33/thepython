...
-
- 不过我一直觉得上面的例子不太好，为了能够让你对编程有更多的第一印象，我参照 `C 语言` 的形式改写了上面的例子：
	- ```python
	  # hello_world.py ①
	  from rich import print  # ②
	  ...
	  ```
		...
		- ③ `print` 这个工具是一个 [[内置]] 工具，属于开箱即用。如果我们想构建自己的工具，可以使用 `def` [[关键字]]。例如此处我们定义了一个工具叫做 `main`。`()`里啥都不写表示使用我们的工具不需要参数。后面跟着的一个 `:` 表示我们当前
